...
public final class BluetoothEventManager {
	...
	private final Collection<BluetoothCallback> mCallbacks = new ArrayList<BluetoothCallback>();
	...
	// This can't be called from a broadcast receiver where the filter is set in
	// the Manifest.
	...
	private class DeviceFoundHandler implements Handler {
		public void onReceive(Context context, Intent intent, BluetoothDevice device) {
			...
			// Skip for now, there's a bluez problem and we are not getting
			// uuids even for 2.1.
		...
		public void onReceive(Context context, Intent intent, BluetoothDevice device) {
			...
			if (bondState == BluetoothDevice.BOND_NONE) {
				if (device.isBluetoothDock()) {
					...
					LocalBluetoothPreferences.removeDockAutoConnectSetting(context,
							device.getAddress());
				...
				int reason = intent
						.getIntExtra(BluetoothDevice.EXTRA_REASON, BluetoothDevice.ERROR);
