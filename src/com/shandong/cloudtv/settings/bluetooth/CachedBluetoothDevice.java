 ...
 * etc.).
 */
public final class CachedBluetoothDevice implements
		Comparable<CachedBluetoothDevice> {
	...
	// how many times did User reject the connection to make the rejected
	// persist.
	...
	void onProfileStateChanged(LocalBluetoothProfile profile,
			int newProfileState) {
		if (Utils.D) {
		...
		if (newProfileState == BluetoothProfile.STATE_CONNECTED) {
			if (!mProfiles.contains(profile)) {
				...
				if (profile instanceof PanProfile
					...
					// Device doesn't support NAP, so remove PanProfile on
					// disconnect
	...
	public CachedBluetoothDevice(Context context,
			...
			LocalBluetoothProfileManager profileManager, BluetoothDevice device) {
		mContext = context;
	...
	private void connectWithoutResettingTimer(boolean connectAllProfiles) {
		...
		for (LocalBluetoothProfile profile : mProfiles) {
			if (connectAllProfiles ? profile.isConnectable() : profile
					.isAutoConnectable()) {
	...
	public void unpair() {
		...
		if (state != BluetoothDevice.BOND_NONE) {
			...
			if (dev != null) {
				...
				if (successful) {
					if (Utils.D) {
						Log.d(TAG, "Command sent successfully:REMOVE_BOND "
								+ describe(null));
					}
				} else if (Utils.V) {
					Log.v(TAG,
							"Framework rejected command immediately:REMOVE_BOND "
									+ describe(null));
				}
			}
		}
	...
	public void clearProfileConnectionState() {
		if (Utils.D) {
			Log.d(TAG,
					" Clearing all connection state for dev:"
							+ mDevice.getName());
		}
	...
	private boolean updateProfiles() {
		...
		mProfileManager.updateProfiles(uuids, localUuids, mProfiles,
				mRemovedProfiles, mLocalNapRoleConnected, mDevice);
	...
	void onUuidChanged() {
		...
		if (DEBUG) {
			Log.e(TAG,
					"onUuidChanged: Time since last connect"
	...
	public int compareTo(CachedBluetoothDevice another) {
		...
		int comparison = (another.isConnected() ? 1 : 0)
				- (isConnected() ? 1 : 0);
		if (comparison != 0)
			return comparison;

		// Paired above not paired
		comparison = (another.getBondState() == BluetoothDevice.BOND_BONDED ? 1
				: 0) - (getBondState() == BluetoothDevice.BOND_BONDED ? 1 : 0);
		if (comparison != 0)
			return comparison;
	...
	void setPhonebookPermissionChoice(int permissionChoice) {
		...
		SharedPreferences.Editor editor = mContext.getSharedPreferences(
				PHONEBOOK_PREFS_NAME, Context.MODE_PRIVATE).edit();
	...
	private void fetchPhonebookPermissionChoice() {
		SharedPreferences preference = mContext.getSharedPreferences(
				PHONEBOOK_PREFS_NAME, Context.MODE_PRIVATE);
	...
	void setMessagePermissionChoice(int permissionChoice) {
		...
		SharedPreferences.Editor editor = mContext.getSharedPreferences(
				MESSAGE_PREFS_NAME, Context.MODE_PRIVATE).edit();
		if (permissionChoice == ACCESS_UNKNOWN) {
			editor.remove(mDevice.getAddress());
		} else {
			editor.putInt(mDevice.getAddress(), permissionChoice);
	...
	private void fetchMessagePermissionChoice() {
		SharedPreferences preference = mContext.getSharedPreferences(
				MESSAGE_PREFS_NAME, Context.MODE_PRIVATE);
